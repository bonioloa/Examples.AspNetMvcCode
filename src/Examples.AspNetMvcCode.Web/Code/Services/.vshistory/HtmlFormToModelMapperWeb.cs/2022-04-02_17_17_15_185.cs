...
public class HtmlFormToModelMapperWeb : IHtmlFormToModelMapperWeb
{
        ...
        foreach (string key in htmlForm.Keys)
        {
            ...
            switch (tmpKey)
            {
                ...
                case ParamsNames.MessageSubject:
                    model.Subject = tmpValue.HtmlInputSanitize();//necessary to prevent XSS because subject can be used as raw html in other components
                    break;
    ...
    public DataGridViewStateSaveViewModel MapDataGridViewStateSave(
        IFormCollection formDataGridViewSave
        , DataGridViewUsage dataGridViewUsageType
        )
    {
        _logger.LogAppDebug("CALL");

        if (formDataGridViewSave.IsNullOrEmpty() || formDataGridViewSave.Keys.IsNullOrEmpty())
        {
            _logger.LogAppError("no data submitted");
            throw new WebAppException();
        }
        if(dataGridViewUsageType == DataGridViewUsage.Unknown)
        {
            _logger.LogAppError($"provide a valid {nameof(DataGridViewUsage)}, provided {dataGridViewUsageType}");
            throw new WebAppException();
        }

        DataGridViewStateSaveViewModel model = new()
        {
            Type  = dataGridViewUsageType,
        };
        ...
        //ATTENTION: any new input field apart from item initial form 
        //must be considered in a specific case in the following switch
        foreach (string key in formDataGridViewSave.Keys)
        {
            tmpKey = key.Clean();
            tmpValue = formDataGridViewSave[tmpKey];
            tmpValue = tmpValue.Clean();
            switch (tmpKey)
            {
                case ParamsNames.DataGridSaveStateSerialized:
                    model.StateSerialized = tmpValue;
                    break;

                case ParamsNames.DataGridSaveStateDescription:
                    //important to sanitize, this field will be treated as Html and we don't want user to input dangerous stuff
                    model.UserProvidedDescription = tmpValue.HtmlInputSanitize();
                    break;

                case ParamsNames.DataGridSaveStateChooseIfSaveIsForAllProfiles:
                    model.SaveAlsoForAllProfiles = tmpValue.EqualsInvariant(AppConstants.OptionAffirmativeValue);
                    break;

                //do nothing for this cases, 
                //these are only technical key/values to be discarded, already handled and validated by architecture
                case ParamsNames.AntiforgeryToken:
                default:
                    break;
