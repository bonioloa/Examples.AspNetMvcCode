...
public class ReportingDocumentWeb : IReportingDocumentWeb
{
        ...
        )
    {
        ...
        */
        ...
        HashSet<ReportingColumnFeaturesModel> tmpAggregatedColumnsFeatures;
        ...
        foreach (long processId in reportData.RootProcessIdList)
        {
            ...
            tmpAggregatedColumnsFeatures = new ();

            tmpRootBasicSchema = itemsBasicDataSchemasList.First(i => i.ProcessId == processId);

            //add columns for root item main data
            tmpAggregatedTable.Columns.AddRange(tmpRootBasicSchema.DataColumns.ToArray());

            MergeColumnFeatures(
                tmpRootBasicSchema.ColumnFeatureSet
                , ref columnsProgressive
                , ref tmpAggregatedColumnsFeatures
                );
            ...
            //build a complete list of all possible slave processes of current process
            ...
            foreach(long slaveProcessId in tmpSlaveProcesses.OrderBy(sp => sp))
            {
                ...
                tmpAggregatedTable.Columns.AddRange(tmpSlaveBasicSchema.DataColumns.ToArray());
                ...
                {
                    List<StepFormSchema> slaveItemsStepForms = formsSchemasByProcess[slaveProcessId];
                    foreach (StepFormSchema stepForm in slaveItemsStepForms.OrderBy(risf => risf.StepProgressive))
                    {
                        if (stepForm.ColumsModel is null
                            || stepForm.ColumsModel.DataColumns.Count <= 0)
                        {
