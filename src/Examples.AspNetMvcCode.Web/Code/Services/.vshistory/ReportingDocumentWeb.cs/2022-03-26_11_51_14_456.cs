...
public class ReportingDocumentWeb : IReportingDocumentWeb
{
    ...
    {
        /*obiettivo: creare una tabella che contenga le 
         * 
         ...
         * 
         * per ogni gerarchia di concatenazione a partire dagli items root 
         * (N-VOLTE finché non si raggiunge un item collegato senza collegati figli)
        ...
        */
        ...
        foreach (long processId in reportData.RootProcessIdList)
        {
            ...
            //add basic item columns, followed by relative forms, ordering by process
            foreach(long slaveProcessId in tmpSlaveProcesses.OrderBy(sp => sp))
            {
                ReportingAreaSimpleModel tmpSlaveBasicSchema = itemsBasicDataSchemasList.First(i => i.ProcessId == processId);
                tmpAggregatedTable.Columns.AddRange(tmpRootBasicSchema.DataColumns.ToArray());
