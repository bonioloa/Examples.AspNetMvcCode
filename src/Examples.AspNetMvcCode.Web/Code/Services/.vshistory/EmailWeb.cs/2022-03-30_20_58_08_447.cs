...
Non disabilitare l'invio di queste email, perché è necessario essere al corrente di eventuali cambi IP delle macchine"
    ...
    public void SendEmailNotificationChange()
    {
        ...
        if (emailForStepChange.IsOpen && emailForStepChange.ToEmails.IsNullOrEmpty())
        {
            ...
            , new List<EmailNotificationModel>()
                {
                        new EmailNotificationModel()
                        {
                            EmailToList = emailForStepChange.ToEmails,
                            Subject = emailForStepChange.Subject,
                            Body = new HtmlString(emailForStepChange.Content),
                        }
                }
            );
        return;
    ...
    public void SendEmailNotificationInclusion()
    {
        ...
        if (emailForInclusion.ToEmails.IsNullOrEmpty())
        {
        ...
        tmpContent = tmpContent
            .Replace(AppConstants.MailPlhItemDescriptiveCode, emailForInclusion.ItemDescriptiveCode)
            .Replace(AppConstants.MailPlhProcessDescription, emailForInclusion.ProcessDescription.GetStringContent());
            ...
            , new List<EmailNotificationModel>()
                {
                        new EmailNotificationModel()
                        {
                            EmailToList = emailForInclusion.ToEmails,
                            Subject = subject,
                            Body = new HtmlString(tmpContent),
                        }
                }
