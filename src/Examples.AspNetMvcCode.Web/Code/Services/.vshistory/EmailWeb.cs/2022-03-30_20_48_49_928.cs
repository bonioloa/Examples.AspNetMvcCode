...
Non disabilitare l'invio di queste email, perché è necessario essere al corrente di eventuali cambi IP delle macchine"
    ...
    public void SendEmailNotificationChange()
    {
        ...
        SendAppEmails(
            emailConfigTenantLgc.TenantCustomSmtpServer
            , addConfigBcc: true
            , urlReplacementRules: new Dictionary<string, Uri>()
                {
                        {
    ...
    public void SendEmailNotificationInclusion()
    {
        ...
        EmailForInclusionNotificationLgc emailForInclusionNotificationLgc = _logicEmail.GetDataForInclusionNotification();

        if (emailForInclusionNotificationLgc.ToEmails.IsNullOrEmpty())
        {
        ...
        tmpContent = tmpContent
            .Replace(AppConstants.MailPlhItemDescriptiveCode, emailForInclusionNotificationLgc.ItemDescriptiveCode)
            .Replace(AppConstants.MailPlhProcessDescription, emailForInclusionNotificationLgc.ProcessDescription.GetStringContent());
            ...
            , new List<EmailNotificationModel>()
                {
                        new EmailNotificationModel()
                        {
                            EmailToList = emailForInclusionNotificationLgc.ToEmails,
                            Subject = subject,
                            Body = new HtmlString(tmpContent),
                        }
                }
