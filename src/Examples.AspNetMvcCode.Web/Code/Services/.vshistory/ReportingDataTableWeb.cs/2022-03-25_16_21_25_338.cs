...
public class ReportingDataTableWeb : IReportingDataTableWeb
{
       ...
       )
    {
        ...
        string columnGroupName = string.Empty;
        if(!isRootGroup)
        {
            columnGroupName = _localizer[nameof(LocalizedStr.ReportDumpPrimaryLinkedData)];
        }

        reportItemPrimaryInfo.First().CalculatedFields
    ...
    private ReportingAreaSimpleModel GetBasicDataSchema(
        ...
        , bool hasItemEndEditableField
        , ItemCalculatedFieldViewModel calculatedField
        )
    {
        ...
        foreach (var calculatedField in calculatedField.OrderBy(cf => cf.Progressive))
        {
