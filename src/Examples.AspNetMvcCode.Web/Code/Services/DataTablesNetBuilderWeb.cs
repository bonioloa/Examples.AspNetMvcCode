...
public class DataTablesNetBuilderWeb : IDataTablesNetBuilderWeb
{
    ...
    public DataTablesNetViewModel BuildSupervisorSearchResultModel(
        IEnumerable<UserFoundModel> userFoundList
        )
    {
        using IDisposable logScopeCurrentMethod =
            _logger.BeginScope(
                ...
                {
                    { AppLogPropertiesKeys.MethodName, nameof(BuildSupervisorSearchResultModel) }
                });
        ...
        List<SupervisorFoundJsonModel> supervisorFoundJsonList = new();
        List<DataTablesNetColumnJsonModel> dataTableColumnsConfig = new();
        ...
        foreach (UserFoundModel user in userFoundList)
        {
            ...
            tmpUserFound.RoleType = user.UserProfileType.ToString();
        ...
        searchResult.ColumnsJsonObjectSerialized =
            new HtmlString(JsonSerializer.Serialize(dataTableColumnsConfig, WebJsonUtility.JsonObjectModelOptions));

        searchResult.RowsJsonObjectSerialized =
            new HtmlString(JsonSerializer.Serialize(supervisorFoundJsonList, WebJsonUtility.JsonObjectModelOptions));
        ...
        )
    {
        using IDisposable logScopeCurrentMethod =
            _logger.BeginScope(
                new Dictionary<string, object>
                {
                    { AppLogPropertiesKeys.MethodName, nameof(BuildItemSearchResultModel) }
                });
        ...
        searchResult.RowsJsonObjectSerialized =
            new HtmlString(JsonSerializer.Serialize(itemFoundTableDynamicList, WebJsonUtility.JsonObjectModelOptions));
        ...
        searchResult.ColumnDefSimpleTargetsSerialized =
            new HtmlString(JsonSerializer.Serialize(columnDefSimpleTargetsIndexes, WebJsonUtility.JsonObjectModelOptions));


        List<List<string>> orderingModel =
            new()
            {
                new List<string>
                {
        ...
        searchResult.ColumnDefaultOrderingModelSerialized =
            new HtmlString(JsonSerializer.Serialize(orderingModel, WebJsonUtility.JsonObjectModelOptions));
            ...
            )
    {
        ...
        validCalculatedFieldsFound =
                itemFoundList.Where(i => i.CalculatedFields.HasValues()
                             ...
                             .Select(i => i.CalculatedFields).Take(1).ToList();
    ...
    {
        int columnIndex = 0;
        IndexedDictionary<SearchResultItemField, ColumnDefinition> availableColumnsIndexes =
            new()
            {
    ...
    {
        SearchResultItemField tmpItemColumnName =
            columnsItemWithIndexes.First(x => x.Value == columnIndex).Key;
        ...
        switch (tmpItemColumnName)
        {
            ...
            case SearchResultItemField.LinkToModify:
                ...
                tableDataProperties.Add(
                    ...
                    _urlHelper.Action(
                        ...
                        , new Dictionary<string, string>()
                            {
                                {ParamsNames.ItemId , item.Id.ToString()}
                            }
                        ).ToString()
                        );
                break;
            ...
            default:
                throw new PmWebException(
                    $"unhandled item column '{tmpItemColumnName}' found for item {item.Id} during row construction"
                    );
        }
    }
    ...
    private static void BuildRowFormPart(
        ref Dictionary<string, string> tableDataProperties
        , IndexedDictionary<string, int> columnsFormWithIndexes
        , IList<PersonalizationGenericModel> formFieldPersonalization
        , ItemAggregatedInfoModel item
        ...
        )
    {
        if (formFieldPersonalization.IsNullOrEmpty())
        {
            throw new ArgumentException(
                $"personalization definition empty for form fields to show in search, but columns are defined"
                );
        }
        ...
        {
            throw new PmWebException(
                $"no form fields for column '{tmpFormColumnName}' found in item {item.Id}"
                );
        }
